...
def getLoginDetails():
    with sqlite3.connect('database.db') as conn:
        ...
        if 'email' not in session:
            ...
            loggedIn = True
            cur.execute(
                "SELECT userId, firstName, usertype FROM users WHERE email = '" + session['email'] + "'")
            userId, firstName, userType = cur.fetchone()
            cur.execute(
                "SELECT count(productId) FROM kart WHERE userId = " + str(userId))
            noOfItems = cur.fetchone()[0]
    conn.close()
    return (loggedIn, firstName, noOfItems, userType)


@app.route("/")
def root():
    loggedIn, firstName, noOfItems, usertype = getLoginDetails()
    with sqlite3.connect('database.db') as conn:
        cur = conn.cursor()
        cur.execute(
            'SELECT productId, name, price, description, image, stock FROM products')
        itemData = cur.fetchall()
        cur.execute('SELECT categoryId, name, discription FROM categories')
        categoryData = cur.fetchall()
    itemData = parse(itemData)
    return render_template('home.html', itemData=itemData, loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems, categoryData=categoryData, usertype=usertype)


@app.route("/admin")
def title():
    _, _, _, userType = getLoginDetails()
    if userType == "admin":
...
@app.route("/addItemBack", methods=["GET", "POST"])
def addItem():
    _, _, _, userType = getLoginDetails()
    if (userType == 'admin' and request.method == "POST"):
...
@app.route("/addCategoryBack", methods=["GET", "POST"])
def addCategoryBack():
    _, _, _, userType = getLoginDetails()
    if userType == 'admin':
        if request.method == "POST":
            ...
            with sqlite3.connect('database.db') as conn:
                try:
                    cur = conn.cursor()
                    cur.execute(
                        'INSERT INTO categories (name, discription) VALUES (\'' + name + '\',\'' + discription + '\')')
                    conn.commit()
                    msg = "added successfully"
                except:
                    msg = "error occured"
                    conn.rollback()
            conn.close()
            print('INSERT INTO categories (name, discription) VALUES (\'' +
                  name + '\',\'' + discription + '\')')
            print(msg)
            return redirect(url_for('root'))
    else:
...
@app.route("/removeCategoryBack")
def removeCategoryBack():
    _, _, _, userType = getLoginDetails()
    if userType == 'admin':
...
def register():
    if request.method == 'POST':
        ...
        with sqlite3.connect('database.db') as con:
            try:
                cur = con.cursor()
                cur.execute('INSERT INTO users (usertype, password, email, firstName, lastName, address1, address2, zipcode, city, state, country, phone) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                            (usertype, hashlib.md5(password.encode()).hexdigest(), email, firstName, lastName, address1, address2, zipcode, city, state, country, phone))
...
def searchItem():
    ...
    if request.method == 'POST':
        ItemName = request.form["searchQuery"]
        if ItemName is None:
            ItemName = ""
        ...
        with sqlite3.connect('database.db') as con:
            cur = con.cursor()
            cur.execute(
                'SELECT products.productId, products.name, products.price, products.image FROM products WHERE name Like' + "\"%" + ItemName + "%\"")
            data = cur.fetchall()
        con.close()
        data = parse(data)
        return render_template('searchItem.html', data=data, loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems, ItemName=ItemName)
